...
export default async function matureCommissionsJob(container: MedusaContainer) {
  ...
  await packs.matureDueCommissions(
    async (beneficiaryId, commissionId, frozen) => {
      ...
      // going either way. Logged (via the wrapper) rather than in the service,
      // which has no injected logger — the container logger is only reachable
      // from this job. Swallowed so the service backstop stays a backstop.
      await notifyFeedNonfatal(container, 'mature-commissions', {
