...
import withdrawalReconcileJob from '../withdrawal-reconcile';
import { GATEWAY_STALE_AFTER_MS } from '../../modules/packs/gateway-reconcile';
import { withdrawalRefundReference } from '../../modules/packs/gateway-withdrawal';
...
const REQUESTED_AT = new Date(Date.now() - GATEWAY_STALE_AFTER_MS * 24);
const pendingRow = {
  id: 'gpw_1',
  customer_id: 'cus_1',
...
function harness(withdrawal: Record<string, unknown> = pendingRow) {
  const packs = {
    ...
    // differently-filtered listGatewayWithdrawals calls (pending, then
    ...
    listGatewayWithdrawals: jest.fn((selector: Record<string, unknown> = {}) =>
      Promise.resolve(
    ...
    claimWithdrawalStatus: jest.fn().mockResolvedValue(true),
...
// (first seen on the retired gateway's staging). Indistinguishable from a rotated
...
const ambiguous400 = () =>
  new GatewayError(
    'TGPay /api/v2/payout/query: non-JSON response (HTTP 400): Not found',
    [],
    400,
  );
...
describe('withdrawal sweep — an unattributable 400 never refunds', () => {
  ...
  it('a requery settle persists settled amount, net and bank references — never the ledger', async () => {
    ...
    expect(h.packs.claimWithdrawalStatus).toHaveBeenCalledWith(
      expect.objectContaining({
        id: 'gpw_1',
        from: ['pending'],
        to: 'settled',
...
describe('withdrawal sweep — a held row is structurally invisible to it', () => {
  ...
  // loop is the sweep's own query, `listGatewayWithdrawals({status:
  ...
  it('does not sweep held rows — an approval queue would self-cancel', async () => {
    ...
    h.packs.listGatewayWithdrawals.mockImplementation(
...
describe('withdrawal sweep — the state an admin approval could leave ambiguous', () => {
  ...
  // always is — see startWithdrawal), old enough that the sweep's
  ...
  it('refunds exactly once, on the shared anchor, and closes failed', async () => {
    ...
    expect(h.packs.updateGatewayWithdrawals).toHaveBeenCalledTimes(1);
    expect(h.packs.updateGatewayWithdrawals).toHaveBeenCalledWith({
    ...
    expect(receipt).toHaveBeenCalledWith(h.container, {
      ...
      // the `||` vs `??` comment on this line in gateway-withdrawal.ts).
      reference: approvedThenAmbiguousRow.merchant_transaction_id,
    ...
    // gateway-withdrawal.ts) — a crash after the update leaves nothing
