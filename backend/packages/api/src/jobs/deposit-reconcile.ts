...
import {
  ...
  applyDepositOutcome,
  gatewayEnabled,
} from '../modules/packs/gateway-deposit';
import {
  getDepositDetail,
...
import {
  GATEWAY_EXPIRED_RETRY_BATCH,
  GATEWAY_EXPIRED_RETRY_MS,
  GATEWAY_FAST_BATCH,
  GATEWAY_FAST_WINDOW_MS,
...
/**
 * Gateway deposit reconciliation.
 *
...
export default async function depositReconcileJob(container: MedusaContainer) {
  ...
  // isFullSweepDue and GATEWAY_FAST_WINDOW_MS for why the split, not the cron,
  ...
  const pending = await packs.listGatewayDeposits(
    fullSweep
      ? { status: 'pending' }
      : {
          status: 'pending',
          created_at: {
            $gte: new Date(now.getTime() - GATEWAY_FAST_WINDOW_MS),
          },
        },
    {
      take: fullSweep ? GATEWAY_RECONCILE_BATCH : GATEWAY_FAST_BATCH,
  ...
  const revivable = !fullSweep
    ? []
    : await packs.listGatewayDeposits(
        {
          status: 'expired',
          created_at: {
        ...
        // GATEWAY_AMBIGUOUS_GIVEUP_MS already documents: a row aged out by the
  ...
  for (const deposit of outstanding) {
    try {
      ...
      if (!config) {
        logger.warn(
          `[deposit-reconcile] ${deposit.merchant_transaction_id} belongs to gateway "${deposit.gateway}", which is not configured here — skipped`,
        );
        continue;
      }
      try {
        ...
        if (refusal.kind === 'ambiguous') {
          logger.error(
            `[deposit-reconcile] requery refused ${deposit.merchant_transaction_id} with an unattributable 400 (${
          ...
          if (deposit.gateway_transaction_id) {
            ...
            logger.error(
              `[deposit-reconcile] requery says ${deposit.merchant_transaction_id} is unknown, but it HAS gateway id ${deposit.gateway_transaction_id} — refusing to expire it; check merchant credentials`,
            );
          }
        }
      }
      ...
      // occupies a slot in the oldest-first GATEWAY_RECONCILE_BATCH window.
      ...
      if (action.kind === 'quarantine') {
        quarantined += 1;
        logger.error(
          `[deposit-reconcile] ${deposit.merchant_transaction_id} requeried at ${action.amount}, above the RM ${GATEWAY_MAX_RM} deposit ceiling — not credited, not written off; left ${deposit.status} for manual settlement`,
        );
        continue;
      }
      ...
      if (action.kind === 'settle') {
        ...
        // (gateway-deposit.ts), one copy shared with the callback route —
        ...
        if (!outcome.applied) {
          quarantined += 1;
          logger.error(
            `[deposit-reconcile] ${deposit.merchant_transaction_id} requeried at ${action.amount}, which is not a creditable amount (${outcome.reason}) — not credited, not written off; left ${deposit.status} for manual settlement`,
          );
          continue;
        }
        ...
        settled += 1;
        logger.warn(
          `[deposit-reconcile] credited ${deposit.merchant_transaction_id} from a REQUERY, not a callback — the callback for this deposit was never received`,
        );
        continue;
      }
      ...
      logger.error(
        `[deposit-reconcile] ${deposit.merchant_transaction_id} failed: ${
          error instanceof Error ? error.message : String(error)
        }`,
      );
    }
  ...
  if (settled || failed || expired || quarantined) {
    logger.info(
      `[deposit-reconcile] swept ${outstanding.length}: ${settled} settled, ${failed} failed, ${expired} expired, ${quarantined} quarantined`,
    );
  }
}
