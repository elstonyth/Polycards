 ...
 */
export const GATEWAY_STALE_AFTER_MS = 60 * 60 * 1000;
 ...
 */
export const GATEWAY_RECONCILE_BATCH = 50;
 ...
 * (GATEWAY_STALE_AFTER_MS) and an 'expired' row is re-read for seven days
 ...
 */
export const GATEWAY_FAST_WINDOW_MS = 20 * 60 * 1000;

/**
 * Cap for one fast-tier run. Smaller than GATEWAY_RECONCILE_BATCH because the
 ...
 * job.
 */
export const GATEWAY_FAST_BATCH = 20;

/** Minutes between full sweeps (stale pending rows + the 'expired' tier). */
export const GATEWAY_FULL_SWEEP_EVERY_MIN = 10;
 ...
 * and withdrawal-reconcile runs on a ten-minute cron, i.e. exactly
...
export const isFullSweepDue = (
  ...
  now.getTime() - lastFullSweepAt.getTime() >=
    GATEWAY_FULL_SWEEP_EVERY_MIN * 60 * 1000;
...
export function reconcileAction(input: ReconcileInput): ReconcileAction {
  if (input.state === 'success') {
    ...
    if (input.amount > GATEWAY_MAX_RM) {
  ...
  return age > GATEWAY_STALE_AFTER_MS ? { kind: 'expire' } : { kind: 'wait' };
}
 ...
 * for the same reason: gateway-deposit.ts records their D… id the moment
...
export function unknownDepositAction(
  ...
  return now.getTime() - createdAt.getTime() > GATEWAY_STALE_AFTER_MS
    ? { kind: 'expire' }
    : { kind: 'wait' };
}
 ...
 * plain-text 400 WITHOUT it (first seen on the retired gateway's staging), and what
 * an auth failure returns is unrecorded (docs/ops/security-verification-
 * checklist.md item D, still open). GatewayError.definite does not help: it
...
export function classifyRequeryError(error: unknown): RequeryRefusal {
  if (!(error instanceof GatewayError)) return { kind: 'rethrow' };
  if (error.has(TGPAY_NOT_FOUND)) return { kind: 'not-found' };
 ...
 * from becoming its own outage. gateway-deposit.ts:245-249 deliberately leaves
 ...
 * in a GATEWAY_RECONCILE_BATCH-sized, oldest-first window until the sweep does
 ...
 *
 * At the DEFAULT it equals GATEWAY_EXPIRED_RETRY_MS, so a row aged out this way
 ...
 */
export const GATEWAY_AMBIGUOUS_GIVEUP_DEFAULT_MS = 7 * 24 * 60 * 60 * 1000;
 ...
 */
export const GATEWAY_EXPIRED_RETRY_MS = 7 * 24 * 60 * 60 * 1000;
 ...
 */
export const GATEWAY_EXPIRED_RETRY_BATCH = 10;
 ...
 * from listGatewayDeposits, so there is no select list that could omit
 ...
 * load-bearing (plan 094). GATEWAY_STALE_AFTER_MS is an hour of grace for a
 ...
 * row (payments/withdrawals/[id]/approve) was CREATED when the customer
...
export function unknownWithdrawalAction(
  ...
  if (hasGatewayTransactionId) return { kind: 'wait' };
  return now.getTime() - submittedAt.getTime() > GATEWAY_STALE_AFTER_MS
    ? { kind: 'refund' }
    : { kind: 'wait' };
}
...
export const GATEWAY_WD_SLOW_AFTER_MS = 24 * 60 * 60 * 1000;
 ...
 * GATEWAY_WD_SLOW_AFTER_MS above, for a completely different failure mode.
 ...
 * automated backstop.
 *
 * Same 24h bar as GATEWAY_WD_SLOW_AFTER_MS, deliberately not a looser one —
 ...
 */
export const GATEWAY_WD_HELD_STALE_AFTER_MS = 24 * 60 * 60 * 1000;
