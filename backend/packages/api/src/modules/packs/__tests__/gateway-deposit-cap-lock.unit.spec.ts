...
/**
 * createDepositCapped — the per-customer pending-deposit cap
 * (GATEWAY_MAX_RECENT_PENDING_PER_CUSTOMER) is counted and inserted inside ONE
 * customer-locked transaction (#429).
 *
 * WHY THIS FILE EXISTS: the module-level cap tests in gateway-deposit.unit.spec
...
const fakeService = (recentPending = 0) => {
  ...
  const calls = {
    listAndCountGatewayDeposits: jest.fn(
      async (_filter: unknown, _config: unknown, _ctx: unknown) => {
        ops.push('count');
        return [[], recentPending] as [unknown[], number];
      },
    ),
    createGatewayDeposits: jest.fn(async (_data: unknown, _ctx: unknown) => {
      ops.push('insert');
      return [{ id: 'gpd_1' }];
    }),
  };
...
describe('PacksModuleService.createDepositCapped', () => {
  it('locks per customer BEFORE counting, then inserts', async () => {
    const f = fakeService(0);
    const row = await f.svc.createDepositCapped(input, f.ctx);
    ...
    expect(f.em.execute.mock.calls[0][1]).toEqual(['gateway-deposit:cus_1']);
  });

  it('runs the count and the insert on the LOCKED context', async () => {
    const f = fakeService(0);
    await f.svc.createDepositCapped(input, f.ctx);
    ...
    expect(f.calls.listAndCountGatewayDeposits.mock.calls[0][2]).toBe(f.ctx);
    expect(f.calls.createGatewayDeposits.mock.calls[0][1]).toBe(f.ctx);
  });
  ...
  it('scopes the count to this customer, to pending only, and to the window', async () => {
    ...
    const filter = f.calls.listAndCountGatewayDeposits.mock
  ...
  it('returns null and writes NOTHING once the count is at the cap', async () => {
    ...
    expect(row).toBeNull();
    expect(f.calls.createGatewayDeposits).not.toHaveBeenCalled();
    expect(f.ops).toEqual(['lock', 'count']);
  });

  it('allows the request one below the cap', async () => {
    const f = fakeService(input.maxRecentPending - 1);
    await expect(f.svc.createDepositCapped(input, f.ctx)).resolves.toEqual({
      id: 'gpd_1',
    });
    expect(f.calls.createGatewayDeposits).toHaveBeenCalled();
  });
});
