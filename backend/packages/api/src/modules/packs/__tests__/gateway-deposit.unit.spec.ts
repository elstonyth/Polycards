import {
  GATEWAY_DEFAULT_METHOD,
  gatewayEnabled,
  newMerchantTransactionId,
  startDeposit,
} from '../gateway-deposit';

// startDeposit talks to the gateway through the seam in gateway.ts.
...
import {
  GATEWAY_MAX_RECENT_PENDING_PER_CUSTOMER,
  GATEWAY_PENDING_WINDOW_MS,
} from '../gateway-deposit';
...
function harness() {
  const packs = {
    ...
    createDepositCapped: jest.fn().mockResolvedValue({ id: 'gpd_1' }),
    updateGatewayDeposits: jest.fn().mockResolvedValue(undefined),
...
describe('gatewayEnabled', () => {
  it('is off unless explicitly enabled AND the active gateway is configured', () => {
    ...
    expect(gatewayEnabled({ GATEWAY_ENABLED: 'true' })).toBe(false);
    expect(gatewayEnabled({ TGPAY_SECRET_KEY: 'sk' })).toBe(false);
    expect(
      gatewayEnabled({ GATEWAY_ENABLED: 'true', TGPAY_SECRET_KEY: 'sk' }),
    ).toBe(true);
  });
});
...
describe('startDeposit', () => {
  it('writes the pending row BEFORE calling the gateway', async () => {
    const h = harness();
    const order: string[] = [];
    h.packs.createDepositCapped.mockImplementation(async () => {
      order.push('row');
      return { id: 'gpd_1' };
    });
    fakeGateway.script({
  ...
  it('stamps their transaction id on the row after a successful submit', async () => {
    ...
    expect(result.merchantTransactionId).toBe(
      h.packs.createDepositCapped.mock.calls[0][0].data.merchant_transaction_id,
    );
  });
  ...
  it('defaults to the provisioned method and passes the CUSTOMER ip', async () => {
    ...
    expect(fakeGateway.calls.deposits[0]).toMatchObject({
      paymentMethodCode: GATEWAY_DEFAULT_METHOD,
      ipAddress: '1.2.3.4',
  ...
  //
  // The GatewayError case is the one that matters and the one that used to be
  // missing: the client wraps a non-JSON/WAF response in GatewayError with
  ...
  it('rejects an invalid amount before touching the gateway', async () => {
    ...
    expect(fakeGateway.calls.deposits).toEqual([]);
    expect(h.packs.createDepositCapped).not.toHaveBeenCalled();
  });
...
describe('startDeposit — pending-deposit cap', () => {
  it('refuses when the capped insert reports the cap was reached', async () => {
    const h = harness();
    h.packs.createDepositCapped.mockResolvedValue(null);
  ...
  it('proceeds when the capped insert returns a row', async () => {
    ...
    expect(h.packs.createDepositCapped).toHaveBeenCalled();
  ...
  it('hands the service the policy: this customer, the cap, the window', async () => {
    ...
    // locked transaction now — gateway-deposit-cap-lock.unit.spec.ts pins that
    // half. What this file still owns is the policy the module passes down.
    const arg = h.packs.createDepositCapped.mock.calls[0][0];
    expect(arg.data.customer_id).toBe(input.customerId);
    expect(arg.data.status).toBe('pending');
    expect(arg.maxRecentPending).toBe(GATEWAY_MAX_RECENT_PENDING_PER_CUSTOMER);
    ...
    expect(arg.windowMs).toBe(GATEWAY_PENDING_WINDOW_MS);
  });
});
