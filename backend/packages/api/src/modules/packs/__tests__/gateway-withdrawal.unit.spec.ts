...
// startWithdrawal talks to the gateway through the seam in
...
function harness(savedAccounts: unknown[] = [SAVED_ACCOUNT]) {
  const packs = {
    ...
    createGatewayWithdrawals: jest.fn().mockResolvedValue([{ id: 'gpw_1' }]),
    // Idempotency-Key replay lookup. Empty by default — no prior intent.
    listGatewayWithdrawals: jest.fn().mockResolvedValue([]),
...
describe('withdrawalsEnabled', () => {
  it('is off unless BOTH switches are on and the active gateway is configured', () => {
    expect(withdrawalsEnabled({})).toBe(false);
    expect(
      withdrawalsEnabled({
...
describe('startWithdrawal — money ordering', () => {
  it('row, then DEBIT, then gateway — in that exact order', async () => {
    const h = harness();
    const order: string[] = [];
    h.packs.createGatewayWithdrawals.mockImplementation(async () => {
      order.push('row');
      return [{ id: 'gpw_1' }];
    });
  ...
  it('routes the debit through withdrawForCashout with the wd: anchor', async () => {
    ...
    expect(call.merchantTransactionId).toBe(
      h.packs.createGatewayWithdrawals.mock.calls[0][0][0]
  ...
  it("refuses a bank the active gateway cannot pay to BEFORE any debit, in the customer's words", async () => {
    ...
    expect(h.packs.createGatewayWithdrawals).not.toHaveBeenCalled();
    expect(h.packs.updateGatewayWithdrawals).not.toHaveBeenCalled();
    expect(h.packs.withdrawForCashout.mock.calls.length).toBe(0);
  ...
  it('REFUNDS the debit and closes the row when the gateway DEFINITELY refuses', async () => {
    ...
    const [{ failure_reason: reason }] =
      h.packs.updateGatewayWithdrawals.mock.calls[0];
  ...
  it('still leaves the row pending for the sweep when the AMBIGUOUS logger throws', async () => {
    ...
    expect(h.packs.withdrawCreditsWithLedger).not.toHaveBeenCalled();
    expect(h.packs.updateGatewayWithdrawals).not.toHaveBeenCalledWith({
      id: 'gpw_1',
      status: 'failed',
    });
  });
  ...
  it('a LOCKED-gate refusal after the precheck passed closes the row', async () => {
    ...
    expect(h.packs.createGatewayWithdrawals).toHaveBeenCalledTimes(1);
    expect(h.packs.updateGatewayWithdrawals).toHaveBeenCalledWith({
      id: 'gpw_1',
      status: 'failed',
    });
  ...
  it('rejects a SAVED account whose stored details are malformed', async () => {
    ...
    // passing while covering none of gateway-withdrawal.ts:209.
    const malformed = {
      ...SAVED_ACCOUNT,
      accountNumber: 'abc',
...
describe('startWithdrawal — approval threshold (held)', () => {
  afterEach(() => {
    delete process.env.GATEWAY_WD_APPROVAL_ABOVE_RM;
  });
  ...
  it('RM 1,000.00 exactly still auto-submits — the boundary is strictly greater-than', async () => {
    ...
    expect(h.packs.createGatewayWithdrawals.mock.calls[0][0][0].status).toBe(
      'pending',
    );
  });
  ...
  it('GATEWAY_WD_APPROVAL_ABOVE_RM=2000, read per call, lifts an amount that would otherwise hold', async () => {
    process.env.GATEWAY_WD_APPROVAL_ABOVE_RM = '2000';
    const h = harness();
    h.packs.walletSummary.mockResolvedValue(roomyWallet);
  ...
  it('GATEWAY_WD_APPROVAL_ABOVE_RM=60 holds an amount the default 1000 threshold would auto-submit', async () => {
    // positiveIntFromEnv has no floor beyond "positive integer" — proved
    // empirically here rather than only by reading the helper.
    process.env.GATEWAY_WD_APPROVAL_ABOVE_RM = '60';
    const h = harness();
    h.packs.walletSummary.mockResolvedValue(roomyWallet);
  ...
  it('GATEWAY_WD_APPROVAL_ABOVE_RM=0 holds ANY amount — the operator incident stop lever', async () => {
    ...
    // (Can't use RM 1 here: GATEWAY_WD_MIN_RM=50 rejects it before the
    // threshold check ever runs, so the smallest amount that reaches the
    // threshold check is the minimum itself.)
    process.env.GATEWAY_WD_APPROVAL_ABOVE_RM = '0';
    const h = harness();
    h.packs.walletSummary.mockResolvedValue(roomyWallet);
    const result = await start(h, { amount: GATEWAY_WD_MIN_RM });
    expect(fakeGateway.calls.withdrawals).toEqual([]);
    expect(result.status).toBe('held');
  });
...
describe('PacksModuleService.withdrawForCashout', () => {
  beforeEach(() => {
    delete process.env.GATEWAY_WD_DAILY_MAX_RM;
  });
  ...
  it('re-reads the row on the LOCKED transaction before doing anything else', async () => {
    ...
    expect(sql).toContain('FROM gateway_withdrawal');
  ...
  it('honours GATEWAY_WD_DAILY_MAX_RM, read per call', async () => {
    const f = fakeService(fakeWallet(), 0);
    process.env.GATEWAY_WD_DAILY_MAX_RM = '40';
  ...
  it('counts pending+settled+held in the last 24h, excluding this attempt and failed rows', async () => {
    ...
    // names gateway_withdrawal too and comes first.
    const [sql, params] = f.em.execute.mock.calls.find(([q]) =>
      (q as string).includes('SUM('),
    ) as [string, unknown[]];
    expect(sql).toContain('gateway_withdrawal');
  ...
  it('an existing held row inside the window counts toward the cap, else batch-approving a queue of them blows past the 24h ceiling', async () => {
    process.env.GATEWAY_WD_DAILY_MAX_RM = '60';
...
/**
 * PacksModuleService.claimWithdrawalStatus — the mutex behind both
 ...
 * flip with updateGatewayWithdrawals({ selector, data }) instead. That
 ...
 */
describe('PacksModuleService.claimWithdrawalStatus', () => {
  ...
  it('claims with ONE conditional UPDATE, answered by RETURNING', async () => {
    ...
    expect(sql).toMatch(/^UPDATE gateway_withdrawal/);
  ...
  it('binds one placeholder per accepted status — never interpolated', async () => {
    const f = fakeClaim([{ id: 'gpw_1' }]);
    await f.svc.claimWithdrawalStatus(
...
describe('POST /store/credits/withdraw — actor_id guard', () => {
  it('401s a register-phase token (actor_id "") without starting a withdrawal', async () => {
    ...
    // startWithdrawal resolves the packs module before it touches the
...
describe('startWithdrawal — Idempotency-Key', () => {
  it('replays a prior withdrawal instead of debiting and submitting again', async () => {
    const h = harness();
    h.packs.listGatewayWithdrawals.mockResolvedValue([
      {
  ...
  it('does not replay a FAILED prior attempt — the retry is a fresh withdrawal', async () => {
    ...
    expect(h.packs.createGatewayWithdrawals).toHaveBeenCalled();
  ...
  it('replays instead of 500ing when a concurrent insert wins the unique index', async () => {
    ...
    h.packs.listGatewayWithdrawals.mockImplementation(async () =>
      seenPrior
        ? [
            {
  ...
  it('rethrows a duplicate key when no active row is visible', async () => {
    const h = harness();
    h.packs.listGatewayWithdrawals.mockResolvedValue([]);
    h.packs.createGatewayWithdrawals.mockRejectedValue(
      Object.assign(
        new Error('duplicate key value violates unique constraint'),
        {
          code: '23505',
  ...
  it('stores the key on the row so the next retry can find it', async () => {
    ...
    const row = (
      h.packs.createGatewayWithdrawals.mock.calls[0][0] as Record<
        string,
        unknown
      >[]
    )[0];
