...
// The gateway_withdrawal.status domain — single source of truth for the
// model's enum below, the admin route's STATUS_FILTERS
// (api/admin/payments/withdrawals/route.ts), and the service's
...
// GatewayWithdrawal — the outstanding-payout record for the payment
// gateway (method WD). Mirrors GatewayDeposit, inverted: the ledger DEBIT
...
// does not echo it.
export const GatewayWithdrawal = model
  .define('gateway_withdrawal', {
    ...
    // gateway_deposit.net_amount — see that model for why the fee is derived
    // rather than stored and why NULL means "unknown", never "no fee".
    net_amount: model.bigNumber().nullable(),
    // The BANK's references for the transfer (the gateway's own id is
    ...
    //
    // The retired gateway's Payout Verification offered every payout to a
    // verify hook BEFORE executing it, and anything but the literal "success"
    // rejected it (TGPay has no such step — hasPayoutVerify is false — so its
    // rows stay NULL). On a gateway that verifies, NULL therefore
  ...
  .indexes([
    ...
    // startWithdrawal only turns the second request into a clean replay
    ...
    // migration's.
    {
      name: 'UQ_gateway_withdrawal_customer_idempotency_key',
