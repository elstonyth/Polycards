...
export async function POST(
  ...
  if (result.claimed && result.kind === 'voucher') {
    // Non-fatal — the claim is already committed and the response is unchanged.
    // The wrapper logs a failure so a broken notification producer on this
    // money-adjacent path is discoverable instead of silently dropped.
    await notifyFeedNonfatal(req.scope, 'store/rewards/claim', {
