...
function harness(row = heldRow(), debitExists = true, frozen = false) {
  ...
  const order: string[] = [];
  const packs = {
    listGatewayWithdrawals: jest.fn(async (selector: { id?: string }) =>
    ...
    // claimWithdrawalStatus is outside the facet on purpose — it is
    // stubbed only so the guard below can prove neither route still reaches
    // for the bare, unlocked claim.
  } satisfies FakeFacet<
    GatewayWithdrawals & Pick<PacksModuleService, 'claimWithdrawalStatus'>
  >;
...
describe('POST /admin/payments/withdrawals/:id/approve', () => {
  ...
  it('a double approve submits exactly ONCE — the second click loses the claim', async () => {
    ...
    // updateGatewayWithdrawals carrying a status is exactly the unlocked
    // path the brief forbids.
    for (const [arg] of h.packs.updateGatewayWithdrawals.mock.calls) {
...
describe('POST /admin/payments/withdrawals/:id/deny', () => {
 ...
 * packs.claimWithdrawalStatus. Between those two calls a debit
...
describe('the debit decision never happens outside the lock', () => {
  it.each([
    ...
    async (_name, handler) => {
      ...
      expect(h.packs.claimWithdrawalStatus).not.toHaveBeenCalled();
    },
  );
});
