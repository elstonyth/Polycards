...
// GET /admin/payments/withdrawals — operator visibility into the gateway
...
// (jobs/withdrawal-reconcile.ts) is the authoritative repair path —
...
// calls the same refundWithdrawal helper the sweep does, on the same
...
// does not.
//
// `stale` reuses the deposits' window (GATEWAY_STALE_AFTER_MS), read off
 ...
 *  getGatewayWithdrawals) — this fallback only ever fires for a caller that
...
// (proved against a real database, see gateway-reconcile.spec.ts).
...
export async function GET(
  ...
  const [rows, total] = await packs.listAndCountGatewayWithdrawals(
    status === 'all' ? {} : { status },
  ...
  const frozenStates = customerIds.length
    ? await packs
        ...
        .catch((error) => {
          logger.error(
            `[admin-withdrawals] frozen-state lookup failed for ${customerIds.length} customer(s) — degrading to frozen:false for this page: ${
  ...
  const withdrawals = rows.map((r) => ({
    ...
    stale:
      r.status === 'pending' &&
      now - new Date(r.updated_at).getTime() > GATEWAY_STALE_AFTER_MS,
