...
function adminWithdrawalViews(): string[] {
  ...
  const m = src.match(
    /const VIEWS: GatewayWithdrawalView\[\] = \[([\s\S]*?)\];/,
  );
  if (!m) {
    throw new Error(
...
const withdrawal = (i: number, over: Partial<Record<string, any>> = {}) => ({
  ...
  // convention as withdrawal-reconcile.unit.spec.ts's pendingRow).
...
function mkScope(
  ...
  const packs = {
    listAndCountGatewayWithdrawals: async (filter: any, opts: any) => {
      calls.filter = filter;
      calls.opts = opts;
...
describe('GET /admin/payments/withdrawals', () => {
  ...
  it('flags a pending row past the sweep window as stale, fresh ones not', async () => {
    const old = withdrawal(1, {
      updated_at: new Date(Date.now() - GATEWAY_STALE_AFTER_MS - 60_000),
    });
  ...
  it('never marks a settled row stale, however old', async () => {
    const { scope } = mkScope([
      withdrawal(1, {
        status: 'settled',
        created_at: new Date(Date.now() - 10 * GATEWAY_STALE_AFTER_MS),
        ...
        updated_at: new Date(Date.now() - 10 * GATEWAY_STALE_AFTER_MS),
      }),
    ]);
    const { res, out } = mkRes();
...
describe('GET /admin/payments/withdrawals/:id/account', () => {
  const mkRevealScope = (rows: any[]) => {
    const calls: { filter?: any; opts?: any } = {};
    const logger = { info: jest.fn(), warn: jest.fn(), error: jest.fn() };
    const packs = {
      listGatewayWithdrawals: async (filter: any, opts: any) => {
        calls.filter = filter;
        calls.opts = opts;
        return rows;
      },
  ...
  it('is registered on the admin action rate limiter', () => {
    ...
    expect(middlewares).toMatch(
      /matcher: '\/admin\/payments\/withdrawals\/\*\/account',\s*method: 'GET',\s*middlewares: \[adminActionRateLimit\]/,
    );
  });
});
