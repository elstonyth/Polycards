...
//
// This resumes exactly where startWithdrawal stopped: that function
...
// copy: gateway-withdrawal.ts's step-3 comment says money may move "only to
...
export async function POST(
  ...
  // for it (needsCustomerContact), so the tests, whose scope has no customer
  // module, are untouched. Resolved BEFORE the claim below, like every other
  ...
  // startWithdrawal writes it 'held' at step 1 and debits at step 2,
