...
} from '../../src/modules/packs/gateway-deposit';

/**
 * createDepositCapped against a REAL database (#429).
 *
 ...
 * meaningful is pinned in gateway-deposit-cap-lock.unit.spec.ts.
 *
...
const CAP = GATEWAY_MAX_RECENT_PENDING_PER_CUSTOMER;
...
medusaIntegrationTestRunner({
  ...
  testSuite: ({ getContainer }) => {
    ...
    const pendingCount = async (customerId: string): Promise<number> => {
      const [, total] = await packs().listAndCountGatewayDeposits({
        customer_id: customerId,
        status: 'pending',
      });
      return total;
    ...
    it('inserts while the customer is below the cap', async () => {
      ...
      expect(
        await packs().createDepositCapped(capped(customer)),
      ).not.toBeNull();
    ...
    it('refuses and writes nothing once the cap is reached', async () => {
      ...
      expect(await packs().createDepositCapped(capped(customer))).toBeNull();
    ...
    it('does not count a pending row older than the window', async () => {
      ...
      const rows = await seed(customer, CAP);
      // updateGatewayDeposits's generated type excludes created_at (it is
      // ORM-managed on insert) — the same `as never` escape hatch
      // gateway-reconcile.spec.ts uses to backdate rows for its stale-window
      // cases.
      await packs().updateGatewayDeposits(
        rows.map((r) => ({
          id: r.id,
          created_at: new Date(Date.now() - GATEWAY_PENDING_WINDOW_MS - 60_000),
        })) as never,
      );
