...
// Ordering of the admin deposit list against a REAL database.
//
...
const PASSWORD = 'deposit-list-order-password-1'; // gitleaks:allow
const ADMIN_EMAIL = 'deposit-list-order-admin@test.dev';
const CUSTOMER_ID = 'cus_deposit_list_order';
...
medusaIntegrationTestRunner({
  ...
  testSuite: ({ api, getContainer }) => {
    describe('GET /admin/payments/deposits ordering', () => {
      ...
      const list = async (qs: string): Promise<string[]> => {
        const res = await unwrapResponse(
          api.get(`/admin/payments/deposits${qs}`, {
            headers: { authorization: `Bearer ${adminToken}` },
          }),
        );
        expect(res.status).toBe(200);
        return (res.data.deposits as { merchant_transaction_id: string }[]).map(
          (d) => d.merchant_transaction_id,
        );
      };
