    ...
    describe('public profiles (GET /store/profiles/:handle)', () => {
      ...
      it('stats parity: SQL aggregate matches the seeded data across packs/rarities; reward pulls excluded (C1)', async () => {
        ...
        expect(res.data.stats).toEqual({
          ...
          volume: Math.round((3 * MYR(RARE_FMV) + MYR(EPIC_FMV)) * 100) / 100,
          by_rarity: {
            Immortal: 0,
      ...
      it('refuses a display name that is taken, or that is not a URL', async () => {
        ...
        // 422 (DUPLICATE_ERROR), not 409 (CONFLICT): Medusa's error handler
        // overwrites a CONFLICT's message with boilerplate about idempotency
        // keys, so the status is asserted together with the message the user
        // actually needs to read.
        expect(taken.status).toBe(422);
        expect(taken.data.message).toMatch(/already taken/i);
