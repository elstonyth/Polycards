 ...
 *
 * The pack is excluded from the public catalog (backend `free_welcome`
 * category), so this link is how an eligible customer reaches it. Rendered only
 * when the server says the account is eligible, which means it disappears by
 * itself the moment the claim is spent.
 *
 ...
 */
export default function FreePackBadge({ slug }: { slug: string }) {
  ...
  return (
    <Link
      href={`/slots/${encodeURIComponent(slug)}`}
      data-testid="free-pack-badge"
      aria-label="Claim your free welcome pack"
      className="fixed bottom-[calc(4.5rem+env(safe-area-inset-bottom))] right-4 z-40 block transition-transform duration-200 hover:scale-105 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white lg:bottom-6"
    >
      <Image
        ...
        width={112}
        height={146}
        // Decorative-adjacent chrome, but it IS the link's only visible content,
        // so it keeps a real alt; priority is deliberately off (it must never
        // compete with the catalog art for bandwidth).
        className={cn(
