...
try {
  ...
  for (const [name, viewport, reducedMotion] of [
    ...
    // Checkpoint pills must not overlap on a narrow bar.
    const boxes = await panel
      .locator('[role="meter"] ~ span, span:has(> svg)')
      .evaluateAll((els) =>
        els
          .filter((e) => e.className.includes('rounded-full'))
          .map((e) => {
    ...
    console.log(
      `[${name}] checkpoints=${boxes.length} overlap=${overlap} boxes=${JSON.stringify(boxes)}`,
    );
