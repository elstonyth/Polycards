...
const page = await browser.newPage({ viewport: { width: 1600, height: 1500 }, deviceScaleFactor: 3 });
for (const [slug, label] of SLUGS) {
  ...
  await page.screenshot({
    ...
    clip: { x: b.x + 0.28 * b.width, y: b.y + 0.70 * b.height, width: 0.42 * b.width, height: 0.24 * b.height },
  });
  console.log(`${slug}\t${label}\tbox=${Math.round(b.width)}x${Math.round(b.height)}`);
}
await browser.close();
console.log("done");
