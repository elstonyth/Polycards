...
// 1) /claw list — pokemon section
{
  ...
  r.pokemon_tiles = await page.evaluate(() => {
    const want = ["Black Pack", "Diamond Pack", "Mythic Pack", "Legend Pack", "Elite Pack", "Platinum Pack", "Rookie Pack"];
    const body = document.body.textContent || "";
    // count distinct visible pack names in the first (pokemon) section
    const open = [...document.querySelectorAll("button, a")].map((e) => e.textContent?.trim());
    return { presentNames: want.filter((n) => body.includes(n)) };
  });
  r.broken_claw = await brokenImgs(page);
  r.overflow_claw = await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth);
  await page.screenshot({ path: `${OUT}/claw-1440-top.png`, clip: { x: 0, y: 0, width: 1440, height: 800 } });
  await ctx.close();
}
...
for (const slug of ["pokemon-black", "pokemon-diamond"]) {
  ...
  const page = await ctx.newPage();
  await page.goto(`${BASE}/claw/${slug}`, { waitUntil: "networkidle", timeout: 30000 });
