...
const JOBS = [
  { base: "black-pack", file: "black-pack-machine.avif", zones: [
    { kind: "banner",  erase: [29, 70, 15.0, 21.8], mask: false, draw: { text: "Pokenic", align: "center", x: 48.5, y: 18.6, fs: 58, color: "rgb(252,233,208)", glow: ["rgba(255,120,50,0.6)", 16] } },
    { kind: "placard", erase: [39.4, 47.8, 75.6, 78.0], mask: true, draw: { text: "pokenic", align: "left", x: 39.9, baseline: 77.5, fs: 18, color: "rgb(34,30,34)" } },
    { kind: "url",     erase: [37.4, 51.8, 86.2, 89.4], mask: true, draw: { text: "pokenic.com", align: "center", x: 44.6, baseline: 88.6, fs: 14, color: "rgb(236,236,239)" } },
  ] },
  { base: "diamond-pack", file: "diamond-pack-machine.avif", zones: [
    { kind: "banner",  erase: [29, 70, 14.5, 21.6], mask: false, draw: { text: "Pokenic", align: "center", x: 48.3, y: 18.4, fs: 58, color: "rgb(247,249,255)", glow: ["rgba(150,190,255,0.5)", 12] } },
    { kind: "placard", erase: [39.6, 47.8, 74.3, 76.9], mask: true, draw: { text: "pokenic", align: "left", x: 40.0, baseline: 76.4, fs: 18, color: "rgb(34,30,40)" } },
    { kind: "url",     erase: [37.4, 52.0, 85.4, 88.4], mask: true, draw: { text: "pokenic.com", align: "center", x: 44.7, baseline: 87.4, fs: 14, color: "rgb(236,238,242)" } },
...
const inputs = {};
for (const j of JOBS) inputs[j.base] = `data:image/avif;base64,${(await readFile(`${DIR}/${j.file}`)).toString("base64")}`;
...
await page.evaluate(async () => { await document.fonts.ready; const a = await document.fonts.load("700 50px Poppins"); const b = await document.fonts.load("600 30px Poppins"); if (!a.length || !b.length) throw new Error("Poppins failed to load"); });
...
const results = await page.evaluate(async ({ JOBS, inputs }) => {
  ...
  for (const j of JOBS) {
    ...
    for (const z of j.zones) {
      const x0 = Math.round(z.erase[0] / 100 * W), x1 = Math.round(z.erase[1] / 100 * W);
      const y0 = Math.round(z.erase[2] / 100 * H), y1 = Math.round(z.erase[3] / 100 * H);
      const bw = x1 - x0, bh = y1 - y0;
      // BLUR-PATCH erase: downscale hard then upscale (+ small canvas blur) → no legible text, keeps tone.
      const sw = Math.max(2, Math.round(bw / (z.kind === "banner" ? 22 : 14)));
      ...
      ctx.imageSmoothingEnabled = true; ctx.filter = "blur(2px)";
      ...
      if (z.draw) {
        const dr = z.draw;
        let fs = dr.fs;
        if (z.kind === "banner") { const maxW = bw * 0.62; ctx.font = `700 ${fs}px Poppins, sans-serif`; while (ctx.measureText(dr.text).width > maxW && fs > 12) fs -= 1; }
        ctx.font = `${z.kind === "banner" ? 700 : 600} ${fs}px Poppins, sans-serif`;
        ctx.textAlign = dr.align; ctx.fillStyle = dr.color;
        if (z.kind === "banner") {
          ctx.textBaseline = "middle";
          if (dr.glow) { ctx.shadowColor = dr.glow[0]; ctx.shadowBlur = dr.glow[1]; ctx.fillText(dr.text, dr.x / 100 * W, dr.y / 100 * H); }
          ctx.fillText(dr.text, dr.x / 100 * W, dr.y / 100 * H); ctx.shadowBlur = 0;
        } else {
          ctx.textBaseline = "alphabetic";
          ctx.fillText(dr.text, dr.x / 100 * W, dr.baseline / 100 * H);
        }
      }
      if (z.mask) { mc.fillStyle = "#fff"; mc.fillRect(x0, y0, bw, bh); }
    }
...
for (const [base, r] of Object.entries(results)) {
  ...
  n++; console.log(`${base}: rebranded (banner+placard+url${base === "diamond-pack" ? "+refl" : ""}) + mask`);
}
