...
if (!b)
  throw new Error(
    'stage bounding box unavailable (element not visible/laid out)',
  );
...
console.log(
  `stage ${Math.round(b.width)}x${Math.round(b.height)} (ar ${(b.width / b.height).toFixed(3)})  machine ${Math.round(ib.width)}x${Math.round(ib.height)} (ar ${(ib.width / ib.height).toFixed(3)})`,
);
console.log(
  `machine fills: ${Math.round((ib.width / b.width) * 100)}% width, ${Math.round((ib.height / b.height) * 100)}% height`,
);
await browser.close();
