...
const token = (await (await fetch(`${API}/auth/user/emailpass`, {
  method: "POST", headers: { "Content-Type": "application/json" },
  body: JSON.stringify({ email: "admin@pokenic.local", password: "pokenicadmin2026" }),
})).json()).token;
const api = await (await fetch(`${API}/admin/pulls`, { headers: { Authorization: `Bearer ${token}` } })).json();
...
const dom = await page.evaluate(() => ({
  ...
  hasRarityBadge: /Common|Uncommon|Rare|Epic|Legendary/.test(document.body.innerText),
...
ok("ledger_rows_match_api", dom.recentRows === api.pulls.length, `dom ${dom.recentRows} vs api ${api.pulls.length}`);
