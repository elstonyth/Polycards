...
const PAGES = [["pokemon-black", "black"], ["pokemon-diamond", "diamond"]];
...
for (const [slug, tag] of PAGES) {
  ...
  await img.screenshot({ path: `${OUT}/live_premium_${tag}.png` }).catch(async () => { await page.screenshot({ path: `${OUT}/live_premium_${tag}.png` }); });
  console.log(`${tag}: src=${src} box=${box ? Math.round(box.width) + "x" + Math.round(box.height) : "none"}`);
  await ctx.close();
}
await browser.close();
