...
What shipped: `POST /store/phone-verification/start` takes an optional `channel` (`sms` default, `call` = Twilio reads the code aloud; `PHONE_OTP_CHANNELS` in `utils/phone-verification.ts`), and the OTP step shows "Get a call instead" beside "Resend code" on the same 30 s cooldown. TemplateSid is SMS-only and is dropped on a call. **Console prerequisites, both done 2026-09-07:** the Voice channel is enabled on the Verify service `VA9f…` (it was disabled — "API calls for this channel will fail"), and Malaysia is enabled under Voice → Geo permissions (SMS and Voice geo permissions are separate lists). A second Verify service "Pokenic" (`VA2558…`, created 2026-08-28) exists on the account; production uses `VA9f…` only.
