...
**A Google sign-in never asks for a password (operator decision 2026-09-11).** Core refuses to register an email whose customer row already `has_account` and has no linking step, so a password account pressing "Continue with Google" used to be sent off to its password (prod 2026-09-10: two refusals inside a minute, then a password login). `googleCallback` now recovers from exactly that refusal by calling `POST /store/customers/link-google` with the register token; the route reads the email from the Google provider identity (issued only for an `email_verified` id_token — never from the body), finds the registered customer with that email and writes `app_metadata.customer_id`, the same link core writes at registration. Nothing else changes: the account keeps its password and the password branch of the phone-change route, the required-phone gate above still applies to every Google account, and a linked sign-in is not a signup (no referral bind). The callback route now logs every refusal it makes on its own (`origin`, `cancelled`, `expired`) at warn, since those never reach the backend and were invisible.
...
**2026-09-07 — Digi (016) subscribers never receive the Verify SMS; voice fallback shipped.** A customer on `+6016…` reported no code. The whole chain was verified clean: the storefront normalized and sent the exact number, the backend answered 200 with no `[phone-otp]` warning, and Twilio's Verify log showed the verification created and every delivery attempt as **Delivered, no error code** — yet the user never saw it. Across 30 days of Verify logs every `+6016` destination (9 verifications, 4 distinct numbers) expired with zero check attempts, while the other Malaysian carriers approved 22 of 35. Nothing in the account was misconfigured (primary compliance profile Approved, SMS geo permissions = Malaysia only, Fraud Guard on, 0 blocked verifications, default template). The break is between Twilio's route and Digi — Twilio's Malaysia guideline requires an `RM0` header plus brand name in every SMS, and the Verify default template preview carries neither; whether that is the mechanism is Twilio support's to confirm, it is not a console toggle.

What shipped: `POST /store/phone-verification/start` takes an optional `channel` (`sms` default, `call` = Twilio reads the code aloud; `PHONE_OTP_CHANNELS` in `utils/phone-verification.ts`), and the OTP step shows "Get a call instead" beside "Resend code" on the same 30 s cooldown. TemplateSid is SMS-only and is dropped on a call — so a voice-delivered code carries no statement of which flow it authorises; the purpose-naming template is the compensating control §Accepted Ceilings relies on, and the voice channel does not have it (recorded 2026-09-10; refusing `call` for `password-reset` is an open operator decision, plans/README round 15). **Console prerequisites, both done 2026-09-07:** the Voice channel is enabled on the Verify service `VA9f…` (it was disabled — "API calls for this channel will fail"), and Malaysia is enabled under Voice → Geo permissions (SMS and Voice geo permissions are separate lists). A second Verify service "Pokenic" (`VA2558…`, created 2026-08-28) exists on the account; production uses `VA9f…` only.
