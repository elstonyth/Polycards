...
| Net deletion on `service.ts`       | `git diff --stat` on `service.ts`, this change only                                                                | net deletion > 0 and within a reasonable margin of N, where **N = total lines landing in `challenge.ts`/`backfills.ts`/`pulled-value.ts`** (measured via `wc -l` on those new files — NOT service.ts's own before/after difference, which is circular against itself); the gap between N and the net deletion is the thin forwarders staying behind. NOT an absolute `wc -l` floor (concurrent work on master can add lines during the extraction). If `pulled-value.ts`'s content is folded into an existing file instead (Scope's executor-judgment fallback, e.g. `pricing.ts`), use that file's own diff-insertion count for its share of N, not `wc -l` — `wc -l` on a pre-existing file counts content this change didn't add. |
...
**Verify**: full unit + modules + smoke green with counts equal to Step 1; `git diff --stat service.ts` (this change only, per plan 111's relative criteria — see "Commands you will need") shows a net deletion within a reasonable margin of N, where N = total lines landing in `challenge.ts`/`backfills.ts`/`pulled-value.ts` (measured via `wc -l` on those new files, independently of service.ts's own diff — not an absolute `wc -l` floor, and not service.ts's baseline-minus-after, which would be circular against itself). If `pulled-value.ts`'s content is folded into an existing file instead (Scope's executor-judgment fallback, e.g. `pricing.ts`), use that file's own diff-insertion count for its share of N, not `wc -l` — `wc -l` on a pre-existing file counts content this change didn't add. `git diff service.ts` contains ONLY deletions, imports, and thin decorated forwarders (decorator + signature + em-resolve + call — no logic edits beyond that shape).
...
- [ ] `service.ts` shrank by a net deletion (`git diff --stat`, this change only) within a reasonable margin of **N = total lines landing in `challenge.ts`/`backfills.ts`/`pulled-value.ts`**, measured independently via `wc -l` on those new files — **not** computed as service.ts's own baseline-minus-after (that comparison is circular against itself and would always pass, which is the trap this exact criterion fell into before plan 111 caught it). If `pulled-value.ts`'s content is folded into an existing file instead (Scope's executor-judgment fallback, e.g. `pricing.ts`), use that file's own diff-insertion count for its share of N, not `wc -l` — `wc -l` on a pre-existing file counts content this change didn't add. The gap between N and the actual net deletion is expected, accounted for by the thin decorated forwarders left behind. This replaces the old absolute-line-count target (plan 111, 2026-08-18). Challenge/backfill logic lives in the new siblings.
