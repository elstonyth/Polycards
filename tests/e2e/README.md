...
| `rewards.spec.ts`          | **SUSPENDED 2026-07-29 — whole file `describe.skip`.** Drove `/vip` + `/daily`, both deleted with the reward-surface suspension (see `docs/superpowers/specs/2026-07-29-suspend-vip-referral-surfaces-design.md`). Un-skip when those surfaces return.                                   |
| `ship-orders.spec.ts`      | Admin advances a delivery order `requested → processed → ready_to_ship → shipped` via the All Orders dashboard (shipping requires a tracking number). Asserted in the UI (modal closes) and server-side (admin delivery-orders API reports `shipped`).                                   |
