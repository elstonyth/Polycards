...
the gateway mirror.** Everything GlobePay tells us about the _cost and identity_
...
the gateway. `gateway_transaction_id` is _their_ id, not the _bank's_.
...
_aggregate_ check that catches the case no single write can see: a settled
...
way to _prove_ that from its own data — which is precisely what B5's
...
_logged_ (`route.ts:263`) and then lost with the log. Low practical risk (we
...
reach the account. Correct as designed. The _gateway-side_ record therefore
...
- **E1** — an ambiguous requery refusal on a _withdrawal_ waits forever; there is
...
surfaced on the admin Deposits/Withdrawals lists) and
