...
- **SECRET ODDS contract:** per-card weights are never exposed to customers. `GET /store/packs/:slug` must omit `weight_2`/`weight_3` exactly as it omits `weight` (`api/store/packs/[slug]/route.ts:19-25`; `src/lib/data/packs.ts:161-166`). Storefront-displayed odds remain the admin-authored `published_odds` — no per-group disclosure change. **SUPERSEDED 2026-08-05 (PR #372):** the route now also publishes `demo_odds`, set 3's TIER split, for the guest demo spin. Per-card weights are still never exposed, and a set 3 indistinguishable from set 1 publishes nothing — see `docs/adr/0005-demo-spin-publishes-set-3-tier-odds.md` before treating this line as the current contract.
